...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if ($con) {
    $query = "SELECT * FROM buslist WHERE dept_place = '$from' AND arr_place = '$to'";
    ...
    if ($count == 0) {
      echo '<script>
                    alert("No bus found in this route.");
...
<body>
  ...
  <div class="mt-20 w-full h-full mb-10">
    <div class="flex items-center justify-center mx-10">
      <table class="table-fixed h-full w-full">
        <tr style="border-bottom: 25px solid transparent;">
          <td class="text-xl font-semibold ">Operator</td>
          <td class="text-xl font-semibold ">Dept. Time</td>
          <td class="text-xl font-semibold ">Arr. Time</td>
          <td class="text-xl font-semibold ">Seat Available</td>
          <td class="text-xl font-semibold ">Fare</td>
          ...
          <tr style="border-bottom: 25px solid transparent;">
            <td>
              <ul class="text-left" style="list-style-type:none;">
                <li class="mb-3" style="color: green; font-weight: bold; font-size: 21px;">
                  <?php echo $row['name']; ?>
                </li>
                <li class="mb-3 text-gray-400"><?php echo $row['type']; ?></li>
                <li class="mb-3 text-gray-400"><b class="text-black">Route:</b><?php echo $row['route']; ?></li>
                <li class="mb-3 text-gray-400"><b class="text-black">Departure Place:</b><?php echo $row['dept_place']; ?>
                </li>
                <li class="mb-3 text-gray-400"><b class="text-black">Arrival Place:</b><?php echo $row['arr_place']; ?>
                </li>
              </ul>
            </td>
            ...
            <td><a href="seat1.php?busid=<?php echo $id; ?>& date=<?php echo $date ?>"
                class="bg-[#3FC495] px-2 py-2 rounded text-white font-semibold cursor-pointer">View Seat</a></td>
          </tr>
          <?php
          $row = mysqli_fetch_array($result); // Fetch next row
        }
        ?>
      </table>
    </div>
